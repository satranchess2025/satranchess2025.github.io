(function () {
  ...
  function flipName(n) {
    if (!n) return "";
    const i = n.indexOf(",");
    return i === -1 ? n.trim() : n.substring(i + 1).trim() + " " + n.substring(0, i).trim();
  }
    ...
    parseComment(text, pos, ctx) {
      ...
      for (let p = 0; p < parts.length; p++) {
        ...
        if (ctx.type === "variation") {
          ...
          // next move after comment starts new mainline paragraph
          ctx.container = null;
        }
      ...
      // A comment (main or variation) is an interruption for move-numbering
    ...
    parse(text) {
      ...
      while (i < text.length) {
        ...
        if (MOVE_NUMBER_REGEX.test(tok)) {
          // skip literal PGN move numbers like "8..."
          continue;
        }
        ...
        if (!isSAN) {
          ...
          if (letters) {
            if (ctx.type === "variation") {
              ...
              this.wrapper.appendChild(P);
              ctx.container = null;
              // mainline text token as stand–alone comment: don't break numbering
              ctx.lastWasInterrupt = false;
            }
          } else {
            this.ensure(ctx, ctx.type === "main" ? "pgn-mainline" : "pgn-variation");
